...
params.contribution_index = params.crowding_index + 1; % Contribution (both domination and hypervolume) index
...
%% Evolutionary cycle
for gen=1:params.gen_max * params.pop_size
    %% Crossover
...
pop = zeros(params.pop_size, params.contribution_index);
...
% Generate two offpsrings (SBX crossover and polynomial mutation)
% Get the i-th couple
parent_index_1 = parent_indexes(1);
parent_index_2 = parent_indexes(2);
...
% Randomly choose and evaluate a single offspring 
if rand() < 0.5
child=evaluate_individual(child_1,params);
else
child=evaluate_individual(child_2,params);
end

% Add a single offspring to the population
pop=[pop;child];
end
...
%pop = pop(1:params.pop_size,:);
	...
	% Obtain the rank of the last front is obtained
	worst_rank = pop(params.pop_size + 1,params.rank_index);

	% Find solutions of the last front
	last_front =find(pop(:,params.rank_index) == worst_rank);
    last_front = pop(last_front,:);
    
    % If all solutions have the same rank
    if size(pop) == size(last_front)
        % Use hypervolume contribution criterion
        pop=compute_hv_contribution(pop,params);
        
        % Sort according contribution
        pop=sortrows(pop,params.contribution_index,'descend');
        
        %Remove worst solution (the one with the highest value of hypervolume contribution)
        pop=pop(1:size(pop)-1,:);
    else
        % Use domination contribution criterion
        
        % Compute domination contribution
        last_front=compute_domination_contribution(last_front, pop, params);
        
        % Remove last front from population
        pop=pop(1:params.pop_size + 1 - size(last_front),:);
        
        % Sort according contribution
        last_front=sortrows(last_front,params.contribution_index,'ascend');
        
        % Remove worst solution from last front (the one with the highest count of dominance)
        last_front=last_front(1:size(last_front)-1,:);
        
        % Include the last front remaining solutions in the population
        if size(last_front)>0
            pop = [pop; last_front];
        end
    end
end
...
function front=compute_domination_contribution(front, pop, params)
for i = 1:size(front)
    count_s = 0;
    for j = 1:size(pop)
        if strongly_dominates(pop(j,:),front(i,:), params)
            count_s=count_s+1;
        end
    end
    front(i,params.contribution_index) = count_s;
end
end

function pop=compute_hv_contribution(pop,params)
% Sort in descending with respect to J2
pop=sortrows(pop,params.fo_index + 1,'descend');

pop(1,params.contribution_index) = Inf;
pop(size(pop),params.contribution_index) = Inf;

for i = 2:size(pop)-1
    pop(i,params.contribution_index) = ( pop(i+1,params.fo_index) - pop(i,params.fo_index) )*( pop(i-1,params.fo_index + 1) - pop(i,params.fo_index + 1));
end
end

%% Strong dominance
function res=strongly_dominates(a, b, params)
s = 0;
