...
fprintf('====== Evolucion Diferencial current-to-best/1/exp (run %d) ======\n',RUN);
fprintf('*** Parametros ***\n');
fprintf('CR = %f\n',CR);
fprintf('F = %f\n',F);
fprintf('K = %f\n',K);
...
while g<G_MAX
    ...
    for i=1:NP
        ...
        j=randi(D);
        doWhile=1;
        while ((l<=D && rand()<CR) || doWhile==1)
            pop(i,j,g+1)=pop(i,j,g)+K*(pop(rbest,j,g)-pop(i,j,g))+F*(pop(r1,j,g)-pop(r2,j,g));
