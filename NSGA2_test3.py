...
while(gen_no < max_gen):
    ...
    for i in range(0, len(non_dominated_sorted_solution)):
        crowding_distance_values.append(crowding_distance(
            function1_values[:], function2_values[:], non_dominated_sorted_solution[i][:]))
    solution2 = solution[:]
    ...
    for i in range(0, len(non_dominated_sorted_solution2)):
        crowding_distance_values2.append(crowding_distance(
            function1_values2[:], function2_values2[:], non_dominated_sorted_solution2[i][:]))
    new_solution = []
    for i in range(0, len(non_dominated_sorted_solution2)):
        non_dominated_sorted_solution2_1 = [index_of(
...
# FUNCION PARA CAMBIAR LABLES DEL CURSOR


def cursor1_annotations(sel):
    sel.annotation.set_text(
        'ISE: {:.2f} \nIADU: {:.2f}'.format(sel.target[0], sel.target[1]))
...
# HOVER
nair_scatter = plt.scatter(function1, function2, c="blue", s=3)
