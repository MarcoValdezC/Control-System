...
def main(function, limites, poblacion, f_mut, recombination, generaciones):
    ...
    population[0] = li[:, 0] + \
        np.random.rand(poblacion, D) * (li[:, 1] - li[:, 0])
    ...
    # plt.figure(1)
    # plt.title('Aproximacion al frente de Pareto gen)
...
for r in range(30):
    ...
    # llamado de la función main de DE
    var = main(pendulum_s, limit, poblacion,
               f_mut, recombination, generaciones)
    x = var[:, 0]
    y = var[:, 1]

    # filename = "PF"+str(r)+".csv"
    # myFile = open(filename, 'w')
    # myFile.write("PF \n")
    # for v in range(len(x)):
    #     myFile.write(str(x[v])+","+str(y[v])+"\n")
    # myFile.close()
